...
**Tests promoted to real files (see `CLAUDE.md`'s "Testing convention"):** the `/tmp/backlog-test-N.sh` scripts described inline throughout this plan (Tasks 1-4's "Step 1" blocks) were throwaway by design and were never part of the repo. They're now committed at `tests/backlog/test-N-*.sh`, runnable via `tests/backlog/run-all.sh`, including a 5th test for the required-fields behavior above. The bash blocks below are kept as a historical record of what each task originally wrote and why — `tests/backlog/` is the source of truth going forward, not this file.
